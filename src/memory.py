...
class MemoryModule:
    """
    Stores user preferences, past interactions, and learning interests within a session
    to enable personalized responses. In a real application, this might persist
    across sessions using a database.
    """

    def __init__(self):
        self.user_preferences: Dict = {}
        self.past_interactions: List[Dict] = []
        self.learning_interests: List[str] = []

    def update_preferences(self, new_preferences: Dict):
        """Updates user preferences."""
        self.user_preferences.update(new_preferences)
        print(f"DEBUG: User preferences updated: {self.user_preferences}")

    def add_interaction(self, interaction_details: Dict):
        """Adds a record of a past interaction."""
        self.past_interactions.append(interaction_details)
        print(f"DEBUG: Past interaction added: {interaction_details}")

    def add_learning_interest(self, interest: str):
        """Adds a new learning interest if not already present."""
        if interest not in self.learning_interests:
            self.learning_interests.append(interest)
            print(f"DEBUG: Learning interest added: {interest}")

    def get_preferences(self) -> Dict:
        """Retrieves current user preferences."""
        return self.user_preferences

    def get_past_interactions(self) -> List[Dict]:
        """Retrieves all past interactions."""
        return self.past_interactions

    def get_learning_interests(self) -> List[str]:
        """Retrieves all learning interests."""
        return self.learning_interests
...
class AgentExecutor:
    ...
    def __init__(self, gemini_api_key: str = ""):
        ...
        self.memory = MemoryModule()  # Initialize the MemoryModule
    ...
    async def execute_request(self, user_query: str) -> Dict:
        ...
        try:
            ...
            if details:
                self.memory.update_preferences(details)

            # Add learning interest if intent is upskill and interest is present
            if intent == "upskill" and details.get("interest"):
                self.memory.add_learning_interest(details["interest"])

        except json.JSONDecodeError:
            print(f"ERROR: Could not parse Gemini response: {gemini_response['text']}")
            # Add interaction even if parsing fails
            self.memory.add_interaction({"query": user_query, "response": "Error: Could not understand request."})
            return {"status": "error", "message": "Could not understand your request. Please try again."}

        response_message = ""
        recommended_resources = []

        # Incorporate memory for personalized responses (example: using stored preferences)
        current_preferences = self.memory.get_preferences()
        print(f"DEBUG: Current memory preferences: {current_preferences}")

        # Merge extracted details with remembered preferences, giving precedence to current details
        combined_preferences = {**current_preferences, **details}

        if intent == "shelter":
            recommended_resources = self.matcher.match_shelter(combined_preferences)
            ...
            recommended_resources = self.matcher.match_jobs(combined_preferences)
            if recommended_resources:
            ...
            recommended_resources = self.matcher.match_upskilling(combined_preferences)
            if recommended_resources:
        ...
        self.memory.add_interaction({
            "query": user_query,
            "intent": intent,
            "details_extracted": details,
            "recommended_resources": [r["id"] for r in recommended_resources]  # Store IDs for brevity
        })
...
async def main():
    ...
    print(f"Current preferences after shelter request: {executor.memory.get_preferences()}")
    print(f"Past interactions after shelter request: {executor.memory.get_past_interactions()}")
    ...
    print(response["message"])
    print(f"Current preferences after job request: {executor.memory.get_preferences()}")
    print(f"Past interactions after job request: {executor.memory.get_past_interactions()}")
    ...
    print(response["message"])
    print(f"Current preferences after upskill request: {executor.memory.get_preferences()}")
    print(f"Learning interests: {executor.memory.get_learning_interests()}")
    print(f"Past interactions after upskill request: {executor.memory.get_past_interactions()}")

    print("\n--- Test Case 4: General Query ---")
    response = await executor.execute_request("What kind of help can you provide?")
    print(response["message"])
    print(f"Past interactions after general query: {executor.memory.get_past_interactions()}")

    print("\n--- Test Case 5: Follow-up request using remembered preferences ---")
    # This request doesn't specify location or pet_friendly, but the memory should retain it
    response = await executor.execute_request("Can you find me another shelter?")
    print(response["message"])
    print(f"Current preferences after follow-up shelter request: {executor.memory.get_preferences()}")
